...
def generate_single_byte_index(opts, crate, name):
    ...
    with mkdir_and_open(crate, name) as f:
        ...
        p("static FORWARD: phf::Map<u8, u16> = phf_map! {")
        for key, value in forward.items():
            p(f"    {key}u8 => {value}u16,")
        p("};")
        p()
        p("#[inline]")
        p("pub fn forward(code: u8) -> u16 {")
        p("    *FORWARD.get(&code).unwrap_or(&0xffff)")
        p("}")

        p()


        p("static BACKWARD: phf::Map<u32, u8> = phf_map! {")
        for key, value in backward.items():
            p(f"    {key}u32 => {value}u8,")
        p("};")
        p()
        p("#[inline]")
        p("pub fn backward(code: u32) -> u8 {")
        p("    *BACKWARD.get(&code).unwrap_or(&0)")
        p("}")
